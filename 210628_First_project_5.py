...
def result_show():
    ...
    i=0
    z=0
    result1_nation=tk.Label(frame6, text="========================================================================")
    result_nation=tk.Label(frame6, text="제주도 : %d명, 미국 : %d명, 일본 : %d명, 태국 : %d명, 대만 : %d명" % (nation_count[0], nation_count[1], nation_count[2], nation_count[3], nation_count[4]))
    result_nation.pack()
    result1_nation.pack()
    for x,y in survey.items():
        ...
        result1_nation.place(x=100, y=200+z)
        result_nation.place(x=100, y=230+z)
        i+=1
        z+=40
        
nation_count=[0,1,1,0,0]        
def information_weather():
    ...
    weather1= tk.Label(frame7, text="%s의 현재 날씨는 %s, %s" % (global_nation,weather_list[city][0],weather_list[city][1]))
    weather1.pack()
    weather1.place(x=120, y=140)

    weather2= tk.Label(frame7, text="%s의 현재 온도는 %d도이며 최저기온 %d도, 최고기온 %d도입니다" % (global_nation,weather_list[city][2], weather_list[city][3],weather_list[city][4]))
    weather2.pack()
    weather2.place(x=120, y=180)

    weather3=tk.Label(frame7, text="%s의 현재 기압은 %d이며 강풍은 %.2fm/s로 불고 있습니다 " % (global_nation,weather_list[city][5], weather_list[city][6]))
    weather3.pack()
    weather3.place(x=120, y=220)

def translate(): #번역정보 제공
    sentence = information_language1.get()
    if global_nation=="제주도":
        dest="ko"
    elif global_nation=="미국":
        dest="en"
    ...
    information_language2.config(text=result.text)
...
rb_vac5=tk.Radiobutton(frame4, text="미국", variable=nation, value="미국", indicatoron=1)
rb_vac5.pack()
rb_vac5.place(x=120, y=330)

rb_vac6=tk.Radiobutton(frame4, text="일본", variable=nation, value="일본", indicatoron=1)
rb_vac6.pack()
rb_vac6.place(x=120, y=360)

rb_vac7=tk.Radiobutton(frame4, text="대만", variable=nation, value="대만", indicatoron=1)
rb_vac7.pack()
rb_vac7.place(x=120, y=390)

rb_vac8=tk.Radiobutton(frame4, text="태국", variable=nation, value="태국", indicatoron=1)
rb_vac8.pack()
rb_vac8.place(x=120, y=420)

rb_vac9=tk.Radiobutton(frame4, text="제주도(해외여행 의향 없음)", variable=nation, value="제주도", indicatoron=1)
rb_vac9.pack()
rb_vac9.place(x=120, y=450)

label_plan=tk.Label(frame4,text="2021년에 해외여행을 갈 계획이 있으십니까?")
label_plan.pack()
label_plan.place(x=100, y=530)

rb_vac10=tk.Radiobutton(frame4, text="해외여행 의향 있음", variable=plan, value="해외여행 의향 있음", indicatoron=1)
rb_vac10.pack()
rb_vac10.place(x=120, y=560)

rb_vac11=tk.Radiobutton(frame4, text="해외여행 의향 없음", variable=plan, value="해외여행 의향 없음", indicatoron=1)
rb_vac11.pack()
rb_vac11.place(x=120, y=590)
...
btn_weather=tk.Button(frame7,text="날씨 정보", command=information_weather)
btn_weather.pack()
btn_weather.place(x=350, y=100, width=80, height=25)        

#5개의 api를 미리 만들어서 딕셔너리에 저장      
weather_list={}
city="Jeju"
...
weather_list["Jeju"]=[data["weather"][0]["main"],data["weather"][0]["description"],data["main"]["temp"], data["main"]["temp_min"],
                              data["main"]["temp_max"], data["main"]["pressure"], data["wind"]["speed"] ]
...
result = requests.get(api)
data = json.loads(result.text)
weather_list["Washington"]=[data["weather"][0]["main"],data["weather"][0]["description"],data["main"]["temp"], data["main"]["temp_min"],
                              data["main"]["temp_max"], data["main"]["pressure"], data["wind"]["speed"] ]
...
result = requests.get(api)
data = json.loads(result.text)
weather_list["Tokyo"]=[data["weather"][0]["main"],data["weather"][0]["description"],data["main"]["temp"], data["main"]["temp_min"],
                              data["main"]["temp_max"], data["main"]["pressure"], data["wind"]["speed"] ]
...
result = requests.get(api)
data = json.loads(result.text)
weather_list["Bangkok"]=[data["weather"][0]["main"],data["weather"][0]["description"],data["main"]["temp"], data["main"]["temp_min"],
                              data["main"]["temp_max"], data["main"]["pressure"], data["wind"]["speed"] ]
...
result = requests.get(api)
data = json.loads(result.text)
weather_list["Taipei"]=[data["weather"][0]["main"],data["weather"][0]["description"],data["main"]["temp"], data["main"]["temp_min"],
                              data["main"]["temp_max"], data["main"]["pressure"], data["wind"]["speed"] ]
print(weather_list)

information_language=tk.Label(frame7,text="※ 언어 공부하기") #언어 번역
information_language.pack()
information_language.place(x=100, y=300)

information_language1=tk.Entry(frame7)
information_language1.pack()
information_language1.place(x=120, y=360, width=250, height=25)

information_language2=tk.Label(frame7)
information_language2.pack()
information_language2.place(x=120, y=420)

btn_language=tk.Button(frame7,text="번역 확인", command=translate)
btn_language.pack()
btn_language.place(x=350, y=300, width=80, height=25)
